...
parser.add_argument(
    "cube", type=str, help="The filename of the image cube that needs to be fixed."
)
parser.add_argument(
    "chan_width",
    type=int,
    help="The number of channels averaged with idlToSdfits when creating the image cube.",
)
...
if not found_writeSdfits:
    print(
        "ERROR: It appears that this cube was not created with the old version of idlToSdfits"
    )
...
if not (1 < args.chan_width < nchan):
    print(
        "ERROR: Please supply a channel width between 1 and the number of channels in the cube."
    )
    sys.exit(2)
...
history_message = "{tool} CRVAL3: {old} to {new}".format(
    tool=scriptname, old=crval3, new=fd[0].header["CRVAL3"]
)
print(
    "Updating {key} from {old} to {new}".format(
        key="CRVAL3", old=crval3, new=fd[0].header["CRVAL3"]
    )
)
...
history_message = "{tool} ALTRPIX: {old} to {new}".format(
    tool=scriptname, old=altrpix, new=fd[0].header["ALTRPIX"]
)
print(
    "Updating {key} from {old} to {new}".format(
        key="ALTRPIX", old=altrpix, new=fd[0].header["ALTRPIX"]
    )
)
fd[0].header.add_history(history_message)
