...
class MappingPipeline:
    ...
    def get_beam_scale(self, receiver, beam_scaling, feed, pol):
        ...
        if receiver == "RcvrArray18_26":
            ...
            self.log.doMessage(
                "ERR", "Beam scaling factors not known for " "receiver:", receiver
            )
    ...
    def getReference(self, scan, feed, window, pol, beam_scaling):
        ...
        for rowNum in rows:
            ...
            if cal_off and cal_on:
                ...
                if (
                    np.all(np.isnan(cal_off.data["DATA"]))
                    or np.all(np.isnan(cal_on.data["DATA"]))
                    or 0 == np.ptp(cal_off.data["DATA"])
                    or 0 == np.ptp(cal_on.data["DATA"])
                ):
                    self.log.doMessage(
                        "DBG",
                        "Bad integration. " "Skipping row",
                        rowNum,
                        "from input data.",
                    )
                    continue
                ...
                cref, tsys, exposure, timestamp, tambient, elevation = (
                    self.sdf.getReferenceIntegration(cal_on, cal_off, beam_scale)
                )
        ...
        avgCref, avgTsys, avgTimestamp, avgTambient, avgElevation, sumExposure = (
            self.cal.getReferenceAverage(
                crefs, tsyss, exposures, timestamps, tambients, elevations
            )
        )

        self.log.doMessage(
            "INFO",
            "Tsys for scan {scan} feed {feed} "
            "window {window} pol {pol}: "
            "{tsys:.1f}".format(
                scan=scan, feed=feed, window=window, pol=pol, tsys=avgTsys
            ),
        )
    ...
    def create_output_sdfits(self, feed, window, pol):
        try:
            ...
            print(
                (
                    "WARNING: Can not find data for scan {scan} window {win} feed {feed} polarization {pol}".format(
                        scan=self.cl.mapscans[0], win=window, feed=feed, pol=pol
                    )
                )
            )
            raise
        ...
        self.outfilename = (
            targetname
            + "_scan_"
            + str(self.cl.mapscans[0])
            + "_"
            + str(self.cl.mapscans[-1])
            + "_window"
            + str(window)
            + "_feed"
            + str(feed)
        ...
        if self.CLOBBER is False and os.path.exists(self.outfilename):
            self.log.doMessage(
                "WARN",
                " Will not overwrite existing pipeline output.\nConsider using '--clobber' option to overwrite.",
            )
            sys.exit()
        ...
        self.outfile[0].write_key(
            "PIPE_VER", PIPELINE_VERSION, comment="GBT Pipeline Version"
        )
    ...
    def multi_tskys(self, crefTime2, refTambient2, refElevation2):
        if (
            (crefTime2 is not None)
            and (refTambient2 is not None)
            and (refElevation2 is not None)
        ):
            return True
        else:
            return False
    ...
    def getReferenceTsky(
        ...
        multiple_reference_scans_for_tsky = self.multi_tskys(
        ...
        if not self.OPACITY:
            self.log.doMessage(
                "DBG", "Getting opacity for reference Tsky: 1st reference"
            )
            ref1_zenith_opacity = self.weather.retrieve_zenith_opacity(
                crefTime1, obsfreqHz, self.log
            )
            if not ref1_zenith_opacity:
                self.log.doMessage(
                    "ERR",
                    "Not able to retrieve reference 1 zenith opacity\n  Please supply a zenith opacity or calibrate to Ta.",
                )
                sys.exit(9)
        else:
            ref1_zenith_opacity = self.OPACITY

        opacity1 = self.cal.elevation_adjusted_opacity(
            ref1_zenith_opacity, refElevation1
        )
        ...
        if multiple_reference_scans_for_tsky:
            ...
            if not self.OPACITY:
                self.log.doMessage(
                    "DBG", "Getting opacity for reference Tsky: 2nd reference"
                )
                ref2_zenith_opacity = self.weather.retrieve_zenith_opacity(
                    crefTime2, obsfreqHz, self.log
                )
                if not ref2_zenith_opacity:
                    self.log.doMessage(
                        "ERR",
                        "Not able to retrieve reference 2 zenith opacity for calibration to:",
                        self.cl.units,
                        "\n  Please supply a zenith opacity or calibrate to Ta.",
                    )
                    sys.exit(9)
            ...
            opacity2 = self.cal.elevation_adjusted_opacity(
                ref2_zenith_opacity, refElevation2
            )
    ...
    def calibrate_fs_sdfits_integrations(self, feed, window, pol, beam_scaling):
        ...
        for scan in self.cl.mapscans:
            ...
            if not cal_switching or not sigref:
                self.log.doMessage(
                    "ERR",
                    "Expected frequency-switched scan",
                    scan,
                    "does not have 2 signal states and 2 noise diode (cal) states",
                )
                self.outfile.close()
            ...
            for chunk in rowchunks:
                ...
                for rowNum in chunk:
                    ...
                    if (
                        sigrefState[0]["cal_on"]
                        and sigrefState[0]["cal_off"]
                        and sigrefState[1]["cal_on"]
                        and sigrefState[1]["cal_off"]
                    ):
                        if (
                            np.all(np.isnan(sigrefState[0]["cal_on"]["DATA"].data))
                            or np.all(np.isnan(sigrefState[0]["cal_off"]["DATA"].data))
                            or np.all(np.isnan(sigrefState[1]["cal_on"]["DATA"].data))
                            or np.all(np.isnan(sigrefState[1]["cal_off"]["DATA"].data))
                            or (
                                0 == np.ptp(sigrefState[0]["cal_off"]["DATA"])
                                and 0 == np.ptp(sigrefState[0]["cal_on"]["DATA"])
                            )
                            or (
                                0 == np.ptp(sigrefState[1]["cal_off"]["DATA"])
                                and 0 == np.ptp(sigrefState[1]["cal_on"]["DATA"])
                            )
                        ):
                            self.log.doMessage(
                                "DBG",
                        ...
                        sigrefState[0]["TP"], sigrefState[0]["EXPOSURE"] = (
                            self.cal.total_power(
                                sigrefState[0]["cal_on"]["DATA"],
                                sigrefState[0]["cal_off"]["DATA"],
                                sigrefState[0]["cal_on"]["EXPOSURE"],
                                sigrefState[0]["cal_off"]["EXPOSURE"],
                            )
                        )
                        sigrefState[1]["TP"], sigrefState[1]["EXPOSURE"] = (
                            self.cal.total_power(
                                sigrefState[1]["cal_on"]["DATA"],
                    ...
                    if self.cl.units != "ta":
                        if not self.OPACITY:
                            intOpacity = self.weather.retrieve_zenith_opacity(
                                intTime, obsfreqHz, self.log
                            )
                            if not intOpacity:
                                self.log.doMessage(
                                    "ERR",
                                    "Not able to retrieve integration "
                                    "zenith opacity for calibration to:",
                                    self.cl.units,
                                    "\n  Please supply a zenith opacity or calibrate to Ta.",
                                )
                                sys.exit(9)
                        ...
                        opacity_el = self.cal.elevation_adjusted_opacity(
                            intOpacity, elevation
                        )

                    if (
                        (self.cl.units == "ta*")
                        or (self.cl.units == "tmb")
                        or (self.cl.units == "jy")
                    ):
                        tastar = self.cal.ta_star(
                            ta, opacity=opacity_el, spillover=self.SPILLOVER
                        )

                    if self.cl.units == "tmb":
                        main_beam_efficiency = self.cal.main_beam_efficiency(
                            self.ETAB_REF, obsfreqHz
                        )
                        tmb = tastar / main_beam_efficiency

                    if self.cl.units == "jy":
                        aperture_efficiency = self.cal.aperture_efficiency(
                            self.ETAA_REF, obsfreqHz
                        )
                    ...
                    if "ta" == self.cl.units:
                        ...
                        self.log.doMessage(
                            "ERR", "units not recognized.  Can not write data."
                        )
                        sys.exit(9)
                    ...
                    row["TUNIT7"] = (
                        self.cl.units.title()
                    )  # .title() makes first letter upper
    ...
    def calibrate_ps_sdfits_integrations(
        self,
        feed,
        window,
        pol,
        ...
        if dtype is None:
            ...
            self.log.doMessage(
                "DBG", "calibrating feed", feed, "window", window, "polarization", pol
            )

        if self.cl.units != "ta" and self.cl.tsky:
            tsky1, tsky2 = self.getReferenceTsky(
                feed,
                window,
                pol,
                crefTime1,
        ...
        for scan in self.cl.mapscans:
            try:
                ...
                self.log.doMessage(
                    "WARN",
                    "{t.bold}WARNING{t.normal}: " "Scan {scan} not found.".format(
                        scan=scan, t=self.term
                    ),
                )
                continue
            ...
            for chunk in rowchunks:
                ...
                for rowNum in chunk:
                    ...
                    if cal_switching and cal_off and cal_on:
                        ...
                        csig, sig_exposure = self.cal.total_power(
                            cal_on["DATA"],
                            cal_off["DATA"],
                            cal_on["EXPOSURE"],
                            cal_off["EXPOSURE"],
                    ...
                    if csig is not None:
                        ...
                        if (avgCref2 is not None) and (crefTime2 is not None):
                            crefInterp = self.cal.interpolate_by_time(
                                avgCref1, avgCref2, crefTime1, crefTime2, intTime
                            )

                            avgTsysInterp = self.cal.interpolate_by_time(
                                avgTsys1, avgTsys2, crefTime1, crefTime2, intTime
                            )
                            if refExposure2:
                                refExposure = (refExposure1 + refExposure2) / 2.0
                            else:
                                refExposure = refExposure1
                            ta, exposure = self.cal.antenna_temp(
                                avgTsysInterp,
                                csig,
                                crefInterp,
                                sig_exposure,
                            ...
                            ta, exposure = self.cal.antenna_temp(
                                avgTsys1, csig, avgCref1, sig_exposure, refExposure
                            )
                        ...
                        if self.cl.units != "ta":
                            ...
                            if self.cl.tsky:
                                if tsky1 and tsky2:
                                    ...
                                    tsky_ref = self.cal.interpolate_by_time(
                                        tsky1, tsky2, crefTime1, crefTime2, intTime
                                    )
                            ...
                            if not self.OPACITY:
                                ...
                                if not intOpacity:
                                    self.log.doMessage(
                                        "ERR",
                                        "Not able to retrieve integration zenith opacity for calibration to:",
                                        self.cl.units,
                                        "\n  Please supply a zenith opacity or calibrate to Ta.",
                                    )
                                    sys.exit(9)
                                    ...
                                    if 0 == outputidx:
                                        self.log.doMessage(
                                            "DBG",
                                            (
                                                "Zenith opacity, win {win} feed {feed} pol {pol} "
                                                "scan {scan} freq {freq} time {time}:".format(
                                                    win=window,
                                                    scan=scan,
                                                    freq=obsfreqHz,
                                                    feed=feed,
                            ...
                            if self.cl.tsky:
                                ...
                                tsky_current = self.cal.tsky(
                                    tambient_current, obsfreqHz, opacity_el
                                )

                                tsky_correction = self.cal.tsky_correction(
                                    tsky_current, tsky_ref, self.SPILLOVER
                                )
                        ...
                        if (
                            ...
                            tastar = self.cal.ta_star(
                                tsrc, opacity=opacity_el, spillover=self.SPILLOVER
                            )
            ...
            if CREATE_PLOTS:
                ...
                averaged_integrations = self.cal.average_spectra(
                    calibrated_integrations, ref_tsyss, exposures
                )
                pylab.plot(
                    averaged_integrations,
                    label=str(scan) + " tsys(" + str(ref_tsyss.mean())[:5] + ")",
                )
    ...
    def calibrate_sdfits_integrations(
        self,
        feed,
        window,
        pol,
        ...
        if avgCref1 is not None:
            self.calibrate_ps_sdfits_integrations(
                feed,
                window,
                pol,
                avgCref1,
