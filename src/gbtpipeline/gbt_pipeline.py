...
def calibrate_win_feed_pol(log, cl_params, window, feed, pol, pipe):
    ...
    if cl_params.refscans:
        ...
        (
            ...
            cl_params.refscans[0], feed, window, pol, cl_params.beamscaling
        )
        ...
        if len(cl_params.refscans) > 1:
            try:
                ...
                log.doMessage(
                    "ERR",
                    "missing 2nd reference scan #",
                    cl_params.refscans[1],
                    "for feed",
                    feed,
                    "window",
                    window,
            ...
            (
                ...
                cl_params.refscans[1], feed, window, pol, cl_params.beamscaling
            )
    ...
    pipe.calibrate_sdfits_integrations(
        feed,
        window,
        pol,
        refSpectrum1,
...
def preview_zenith_tau(log, row_list, cl_params, feeds, windows, pols):
    ...
    if cl_params.zenithtau is None:
        ...
        if zenithtau:
            log.doMessage(
                "INFO", "Approximate zenith opacity for map: {0:.3f}".format(zenithtau)
            )
        else:
            log.doMessage(
                "ERR",
    ...
    # else if set at the command line
        ...
        log.doMessage(
            "INFO", "Zenith opacity for " "map: {0:.3f}".format(cl_params.zenithtau)
        )
...
def calibrate_maps(log, cl_params, row_list, term):
    ...
    if cl_params.refscans:
        log.doMessage(
            "INFO", "Refscan(s):", ",".join([str(xx) for xx in cl_params.refscans])
        )
    if cl_params.mapscans:
        log.doMessage(
            "INFO", "Mapscan(s):", ",".join([str(xx) for xx in cl_params.mapscans])
        )
    else:
        log.doMessage(
            "ERR", "{t.bold}ERROR{t.normal}: " "Need map scan(s).\n".format(t=term)
        )
        sys.exit()
    ...
    for window in windows:
        ...
        for feed in feeds:
            for pol in pols:
                ...
                #  to be calibrated
                CalibratedMap = namedtuple(
                    "CalibratedMap", "mp_object, window, feed, pol, start, end"
                )
        ...
        if maps_for_this_window:
            ...
            for mp, window, feed, pol in maps_for_this_window:
                ...
                if PARALLEL:
                    ...
                    log.doMessage(
                        "DBG",
                        "Feed {feed} Pol {pol} " "started.".format(feed=feed, pol=pol),
                    )
                    calibrate_win_feed_pol(log, cl_params, window, feed, pol, mp)
                    log.doMessage(
                        "DBG",
                        "Feed {feed} Pol {pol} " "finished.".format(feed=feed, pol=pol),
                    )
...
def command_summary(cl_params, term, log):
    ...
    for input_param in cl_params._get_kwargs():
        ...
        elif "feed" == parameter or "pol" == parameter or "window" == parameter:
            if parameter_value is None:
                value = "all"
            else:
...
def calibrate_file(term, log, command_options):
    ...
    if command_options.pol:
        if set(command_options.pol).isdisjoint(set(row_list.pols())):
            ...
            if pol_diff:
                log.doMessage(
                    "WARN", "POL", list(pol_diff), "not in", os.path.basename(indexfile)
                )
    ...
    if command_options.window:
        if set(command_options.window).isdisjoint(set(row_list.windows())):
            ...
            command_options.window = list(
                set(command_options.window).intersection(set(row_list.windows()))
            )
    ...
    if not command_options.mapscans:
        if command_options.refscans:
            log.doMessage(
                "WARN",
                "Referene scan(s) given without map scans, "
        ...
        if maps:
            ...
            log.doMessage(
                "ERR", "No scans specified and found no " "position-switched maps."
            )
            sys.exit()
        ...
        for map_number, map_params in enumerate(maps):
            ...
            log.doMessage(
                "INFO", "\nProcessing map:", str(map_number + 1), "of", len(maps)
            )
...
def runPipeline():
    ...
    if os.path.isdir(cl_params.infilename):
        ...
        if cl_params.mapscans and set(cl_params.mapscans).isdisjoint(
            ...
            log.doMessage(
                "ERR", "no given MAPSCAN(S)", cl_params.mapscans, "in dataset"
            )
            quitcal = True
        if cl_params.refscans and set(cl_params.refscans).isdisjoint(
            ...
            log.doMessage(
                "ERR", "no given REFSCAN(S)", cl_params.refscans, "in dataset"
            )
            quitcal = True
        ...
        for infilename in glob.glob(
            input_directory + "/" + os.path.basename(input_directory) + "*.fits"
        ):
            log.doMessage(
                "DBG",
