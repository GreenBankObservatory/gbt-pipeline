...
parser = argparse.ArgumentParser(
    description="Compute default images from calibrated spectra"
)
parser.add_argument(
    "aipsid", type=int, help="Your *PIPELINE* AIPS number (should always be the same)"
)
parser.add_argument(
    "--average", "-a", type=int, default=1, help="Number of channels to average"
)
parser.add_argument(
    "--map-ra-degrees",
    "-r",
...
spectra = AIPSUVData(
    AIPSCat()[mydisk][0].name,
    AIPSCat()[mydisk][0].klass,
    mydisk,
    AIPSCat()[mydisk][0].seq,
)
...
print("Ra,Dec:", raDeg, decDeg, "Image:", imxSize, imySize, cellsize, end=" ")
...
sdgrd.aparm[1] = math.floor(raDeg / 15.0)
sdgrd.aparm[2] = math.floor(((raDeg / 15.0) - sdgrd.aparm[1]) * 60.0)
sdgrd.aparm[3] = round(
    ((((raDeg / 15.0) - sdgrd.aparm[1]) * 60.0) - sdgrd.aparm[2]) * 60.0
)
...
sdgrd.aparm[5] = math.floor((decDeg - sdgrd.aparm[4]) * 60.0)
sdgrd.aparm[6] = round((((decDeg - sdgrd.aparm[4]) * 60.0) - sdgrd.aparm[5]) * 60.0)
# now deal with degrees and/or minutes == 0
if decSign < 0.0:
    sdgrd.aparm[4] = -1.0 * sdgrd.aparm[4]
    if sdgrd.aparm[4] == 0:
        sdgrd.aparm[5] = -1.0 * sdgrd.aparm[5]
        if sdgrd.aparm[5] == 0:
            sdgrd.aparm[6] = -1.0 * sdgrd.aparm[6]
...
newBmaj = bmaj
if sdgrd.xtype == -12:
    convolveMaj = sdgrd.xparm[2] / 3600.0  # convolving function FWHM in degrees
    ...
    print(bmaj * 3600.0, convolveMaj * 3600.0, "->", newBmaj * 3600.0)
if sdgrd.xtype == -16:
    # Convolved image resolution adds in quadrature
    newBmaj = bmaj
...
restFreqName = "_%.0f_MHz" % (restFreqHz * 1.0e-6)
