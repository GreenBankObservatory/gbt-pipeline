...
if argc < 5:
    ...
    print(
        "usage: tempNH3-11-22 <aipsNumber> <outName> <velocityKmS> <velWidthKmS> <fluxMin>"
    )
...
velocityKmS = 0.0  # Default velocity (km/s)
velWidthKmS = 10.0  # Default velocity Full Width (km/s)
...
restFreq11 = 23694.506 * 1.0e6
restFreq22 = 23722.633 * 1.0e6
# start with 11 line
restFreq = restFreq11

# These constants are particular to each source
sourceLineWidthHz = 0.2e6

sourceFreq = restFreq * (1.0 - (velocityKmS / cLightKmS))
print("Source Frequency:", sourceFreq * 1.0e-6, "+/-", dNu * 1.0e-6, " MHz")
# now set the default line channels and widths
lineWidthChan = 100
lineChan = round(nChan * 0.5)

# Compute number of channels from the Velocity width
if dNu != 0:
    lineWidthChan = round(velWidthKmS * sourceFreq / (dNu * cLightKmS))
    lineWidthChan = round(lineWidthChan / 2.0)
    if lineWidthChan < 0:
...
# make a N(2,2) image
comb.aparm[1] = cLightKmS * (6.0 / 4.0) * 1.55e14 / (restFreq22 * restFreq22 * 1.0e-9)
comb.aparm[2] = 1.0e-10
comb.aparm[3] = 0
comb.aparm[4] = 0
comb.outcl = "N(2,2)"
comb.opcode = "SUM"
...
# make a N(1,1) image
comb.aparm[1] = cLightKmS * (2.0 / 1.0) * 1.55e14 / (restFreq11 * restFreq11 * 1.0e-9)
comb.inclass = "11"
comb.in2class = "11"
comb.outcl = "N(1,1)"
comb.go()
...
comb.in2class = "ONE"
comb.opcode = "OPTD"
comb.aparm[1] = 1.0 / 2.3025851
comb.aparm[2] = 0.0
comb.aparm[3] = 1.0 / 3.0
comb.aparm[4] = 0.0
comb.bparm[1] = 0
comb.bparm[2] = 0
comb.bparm[4] = 0
...
comb.in2class = "ONE"
comb.opcode = "OPTD"
comb.aparm[1] = 1.0 / 2.3025851  # 1. / ln(10.)
comb.aparm[2] = 0.0
comb.aparm[3] = 1.0 / 5.0
comb.aparm[4] = 0.0
comb.outcl = "LOG-22"
comb.go()
...
comb.opcode = "CLIP"
comb.outcl = "T CLIP"
comb.aparm[1] = -1.0  # Minimum temp
comb.aparm[2] = 100.0  # Maximum temp; arbitrary
comb.aparm[3] = 0.0
comb.go()
