; TATM.pro estimates the atmospheric effective temperature based
; on local ground temperature measurements.  These estimates
; come from a model reported by Ron Maddale
; HISTORY
...
pro tatm, freqGHz, tmpC, airTempK

  on_error,2
  if (not keyword_set(freqGHz)) then begin
    print,'usage: tAtm, freqGHz, tmpC, airTempK'
    print,'where: freqGHz  - input frequency in GHz'
    print,'where: tmpC     - input ground temperature in Celsius'
    print,'where: airTempK - output Air Temperature in Kelvin'
    print,'tatm model is provided by Ron Maddalena'
...
;A and B coefficients are:
;
  A = [259.69185966, -1.66599001, 0.226962192, -0.0100909636,  0.00018402955, -0.00000119516 ]
  B = [0.42557717,    0.033932476,0.0002579834,-0.00006539032, 0.00000157104, -0.00000001182]
  ...
  TATM = TATM + (B[0] + B[1]*FREQ + B[2]*FREQ2 + B[3]*FREQ3 + B[4]*FREQ4 + B[5]*FREQ5)*TMPC
;
  if (not keyword_set(airTempK)) then $\
    print,'Model Air Temperature (K): ', TATM
  airTempK = TATM
return
end
