...
# import pipeutils
from gbtpipeline import Pipeutils
...
class Pipe:
    def __init__(self):
        ...
        self.spillover = 0.99  # rear spillover, ohmic loss, blockage (etaL)
        self.gain_coefficients = [0.0910, 0.00434, -5.22e-5, 0]
...
class TestSpectrumOperations(unittest.TestCase):
    ...
    def setUp(self):
        self.calONSig = np.array(
            [0.6152100923665527, 0.500503480229196, 0.14375595767163507]
        )
        self.calOFFSig = np.array(
            [0.8536594292721693, 0.968031662525558, 0.35740514248237065]
        )

        self.calONRef = np.array(
            [0.5017073137242803, 0.982840923900886, 0.4100583472818239]
        )
        self.calOFFRef = np.array(
            [0.43708647917671695, 0.3593699707018171, 0.1075494918610801]
        )
    ...
    def test_aperture_efficiency(self):
        result = self.pipe.aperture_efficiency(0.23, 27e9)
        expected_result = 0.20256449890095335
        assert result == expected_result

    def test_gain(self):
        gain_coefficients = (0.91, 0.00434, -5.22e-5)
        elevation = 60
    ...
    def test_atmospheric_opacity(self):  # eqn. (9) in PS spec
        zenith_opacities = [0.01 * xx for xx in range(1, 9)]
        elevation = 45.0
    ...
    def test_TaStar(self):
        ...
        elevation = 63
        gain = self.pipe.gain((0.91, 0.00434, -5.22e-5), elevation)
        opacity = Pipeutils.corrected_opacity((0.07,), elevation)[0]
        beam_scaling = 1
        result = self.pipe.TaStar(Ta, beam_scaling, opacity, gain, elevation)
        expected_result = np.array(
            [75.49444398748547, 1.864747919874635, -0.5003700906180537]
        )
    ...
    def test_jansky(self):
        ...
        aperture_efficiency = self.pipe.aperture_efficiency(0.23, 27e9)
    ...
    def test_interpolated_reference_scan(self):  # eqn. (6) in PS spec
        ...
        firstRef_timestamp = 1000.0
        secondRef_timestamp = 2000.0

        integration_timestamp = 1350.0

        result = self.pipe.refInterp(
            reference_scan1,
            reference_scan2,
            firstRef_timestamp,
            secondRef_timestamp,
    ...
    def test_interpolated_reference_system_temp(self):  # eqn. (7) in PS spec
        reference_Tref1 = 40.0
        reference_Tref2 = 50.0

        firstRef_timestamp = 1000.0
        secondRef_timestamp = 2000.0

        integration_timestamp = 1500.0
