...
{ 
  ...
  fprintf( stderr, 
	   "idlToSdfits: usage\n");
  fprintf( stderr, 
	   "idlToSdfits [-a <N>] [...] [-o <outName>] [-c <start:stop>] [-s <scanList>] <idlFitsFileName>\n");
  fprintf( stderr, 
  ...
  fprintf( stderr, 
	   "where [-v]               optionally replace channel 2 with beam map\n");
  fprintf( stderr, 
  ...
  fprintf( stderr, 
	   "  Note, channels are averaged before median filtering !\n");
  fprintf( stderr, 
  ...
  fprintf( stderr, 
	   "\nPlease acknowledge use of this program, if the GBT images result\n");
  fprintf( stderr, 
	   "in a publication.   Ie: \n");
  fprintf( stderr, 
	   "    We acknowledge Glen Langston's help in GBT imaging.\n");
  fprintf( stderr, 
	   "Version %s: Date: %s\n", SOFTVERSION, "December 29, 2009" );
  fprintf( stderr, 
...
{ char raStr[32] = "", decStr[32] = "", * errMsg = NULL, fitsName[512] = "",
    calType[512] = "RAW", textName[512]="", charScanList[512] = "",
  ...
  long coordType = J2000, doPrint = FALSE, hasNans = FALSE, doRFI = FALSE,
    nPols = 2, nChan = 0, nScans = 1, nAve = 0, nSum = 0, doXY = FALSE,
    i = 0, skipCount = 101, k = 0, iRow = 0, maxScans = MAXSCANS, 
    ...
    stopChan = 0, doCalOnOff = FALSE, doFlag = FALSE, nBeams = 1, nStates = 1,
    flaggedLast = 0, mjd =  today2mjd(), firstNanChan = 0, lastNanChan = 0,
    zeroNans = FALSE, doBeam = FALSE, doQuickBaseline = FALSE, bChan = 0,
    eChan = 0, doEndTrim = FALSE, nLine = 0, doUser = FALSE, nKeepScans = 0;
  char *names[500], fileName[512] = "SCAN=41", flagStr[1024]="",
    ...
    directoryName[MAXIDLSTRING] = "./", userSources[MAXIDLSTRING] = "";
  GBTIDL gbtIdl, sumX, sumY, firstIdl, lastIdl, tempIdl;
  double lineNu = 0, gain = .69, gainFactor = 1./.69, tSys = 0., tSysAve = 0,
    oMedian = 0, oAve = 0, oRms = 0, oMin = 0, oMax = 0, ave = 0, rms = 0,
    minTSys = 13., noiseLimit = 0., tInt = 0., elMin = 0, elMax = 0,
    aveRa = 0, aveDec = 0, tRx = 14., xs[32767], ys[32767], y2s[32767],
    freqMHz = 1420.4, lineSigma = 10;
  ...
  while (i < argc - 1) {
	...
	gainFactor = 1.0 / gain;
      ...
      i++;
      sscanf( argv[i], "%s", userSources);
      doUser = TRUE;
      fprintf ( stderr, "Reading User Source List: %s\n", userSources);
    }
  ...
  for (iFile = 0; iFile < nScans; iFile++) {
	...
	firstIdl.nStates = 1;
        ...
        if (nNoise > 0) {	
	  strcpy( noiseName, fitsName);
	  stripExtension( noiseName); 
	...
	for (i = 0; i < MAXSCANS; i++) {
      ...
      if (noiseLimit > 0. && rms > noiseLimit) {
  ...
  doPrint = TRUE;
  lineSummary( doPrint, nLine, nChan, maxLines, &bChan, &eChan);
  ...
  if (countX > 0 && countY > 0) 
    ...
    quickPlot( plotName, TRUE, gnuCmds, title, xLabel, yLabel, 
	       sumX.data_points - 1, 
