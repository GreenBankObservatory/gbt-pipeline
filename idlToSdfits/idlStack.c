...
{ char * errMsg = NULL;
  ...
  for (i = 0; i < MAXIDL; i++) {        /* Mark all data as freed */
    ...
    scanIMjds[i] = -1;                  /* no valid integration yet */
...
/* select a scan+integration by the MJD and UTC time of the obs */
{ char * errMsg = NULL;
  long inMilliMjd = (mjd + (utc/TWOPI))*1000.;

  *id = -1;

  /* look throught the iMjd list, this could be optumized, if sorted */
  for (i = 0; i < nIdl; i++) {
    if (inMilliMjd == scanIMjds[i])
      break;
  } /* end for all scans kept */
...
{ long i = 0, n = 0, j = 0, iScan = 0, iMjd = 0;
...
{ long i = 0, n = 0, scan = 0, integration = 0, mjd = 0;
  ...
  *nOut = 0;                                /* assume no idl scans found */
  ...
  for (i = 0; i < nIdl && n < maxN; i++) {
	...
	firstMjd = idls[i]->mjd;
      if (n > 0) {
	if (((idls[i]->mjd - firstMjd) > 1) ||
	    ((firstMjd - idls[i]->mjd) > 1)) { /* two scans not on same day! */
	  fprintf( stderr, "Two observations have the scame scan number!\n");
	}	  
      }
      else {
	outIdls[n] = idls[i];
	...
	n++;
      } /* end if scan number is on more than one day */
    } /* end if a matching scan number */
  } /* end for all idl structures */
  *nOut = n;
  return(NULL);
} /* end of getIdl() */
