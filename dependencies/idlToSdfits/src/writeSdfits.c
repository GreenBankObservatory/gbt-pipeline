...
#define REFTSYS  18.             /* set reference Tsys to 18 K for weight */
...
{ int status = 0, naxis = NRANDOM, extend = TRUE, 
  ...
  double bzero = 0, bscale = 1, az = 0, el = 0, 
  ...
  fprintf( stderr, "Writing date obs %ld -> %s (%s)\n", 
	   mjd, dateStr, mapStr);
...
{ char tempFileName[MAXNAMELENGTH] = "./temp.SDFITS", *errMsg = NULL,
  ...
  long  maxN = MAXPOLS, nIdl = 0, n = 0, i = 0, j = 0, nBytes = 0, k = 0, 
    debugPrint = 0, maxDebugPrint = 5, mjd = 0, iScan = 0, nPol = 0, 
  ...
  double utc = 0, ra2000 = go.ra2000*DEGREE, dec2000 = go.dec2000*DEGREE, 
    az = 0, el = 0, weight = 1, dMjd = 0, dMjdRef = 0.;
  ...
  for (iS = 0; iS < nScans; iS++) {
    ...
    nOut += 4;
    if ( pIdl->tsys > 0.&& pIdl->tintg > 0.)/* weight relative to TSYS * 1 s */
      weight = pIdl->tintg * (REFTSYS/pIdl->tsys) * (REFTSYS/pIdl->tsys); 
    else 
      weight = -1.;                         /* else just flag data */
	...
	for (k = 0; k < nChan; k++) {           /* for all channels */
	  ...
	  dataArray[n+(k*3*nPol)+dN+2] = weight;   /* weight */
  ...
  fprintf( stderr, "\n writeSdfits: %ld data bytes (%ld FITS blocks)\n", 
	   nBytes, nBlocks);    
